...
try:
    ...
    TEXT = "Hi BI Team, Zendesk Ticket Metrics Incremental Load Failed - Database Connection Issue needs to be fixed."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    ...
    job_name = 'Ticket_Metrics_Incremental_Load'
...
try:
    sql_query1 = "select max(max_ticket_sync)::timestamp from src_zendesk.ticket_metrics"
    cur.execute(sql_query1)
    result1 = cur.fetchone()
    max_updated_ticketmetrics = result1[0]

    sql_query2 = "select EXTRACT(EPOCH FROM TIMESTAMP WITH time zone '" + \
        str(max_updated_ticketmetrics) + "') "
    cur.execute(sql_query2)
    result2 = cur.fetchone()
    max_epoch_ticket_metrics = float(result2[0])
    ...
    print('max_updated_ticketmetrics:' + str(max_updated_ticketmetrics))
    print(max_epoch_ticket_metrics)
    print('max_updated_ticket:' + str(max_updated_ticket))
    print(max_epoch_ticket)

    if max_epoch_ticket_metrics == max_epoch_ticket:
        print('no more data')
        update_ticket_ids = []
    else:
        sql_query = "select distinct extract(EPOCH from _fivetran_synced) from src_zendesk.ticket where extract(EPOCH from _fivetran_synced) > " + str(
            max_epoch_ticket_metrics) + " and extract(EPOCH from _fivetran_synced) <= " + str(max_epoch_ticket)
        cur.execute(sql_query)
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Ticket Metrics Incremental Load Failed- could not fetch list of ticket ids."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    s.login(gmail_user, gmail_password)
    s.sendmail("gmail_user", "rainy.li@dynatrace.com", message)
    s.sendmail("gmail_user", "Bussys_bi@dynatrace.com", message)
...
try:
    ...
    for update_ticket_id in update_ticket_id:
        url = f'https://dynatrace.zendesk.com/api/v2/tickets/{update_ticket_id}/metrics.json'
        ...
        df_data['max_ticket_sync'] = max_updated_ticket
        df_data['max_ticket_sync'] = pd.to_datetime(df_data['max_ticket_sync'], utc=True)
except:
    cur.execute(
        "update logging.job_summary_logs set status ='Failed', end_time = current_timestamp where job_execution_id = " + job_exec_id)
    cur.execute(
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Ticket Metrics Incremental Load Failed- API data fetch needs to be fixed."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    s.login(gmail_user, gmail_password)
    s.sendmail("gmail_user", "rainy.li@dynatrace.com", message)
    s.sendmail("gmail_user", "Bussys_bi@dynatrace.com", message)
...
try:
    df_data.to_sql('ticket_metrics', cnx, schema='src_zendesk', method='multi', if_exists='append', index=False, chunksize=5000)
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Ticket Metrics Incremental Load Failed- could not load data to database."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    s.login(gmail_user, gmail_password)
    s.sendmail("gmail_user", "rainy.li@dynatrace.com", message)
    s.sendmail("gmail_user", "Bussys_bi@dynatrace.com", message)
