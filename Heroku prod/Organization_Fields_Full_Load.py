...
SUBJECT = "Zendesk Organization Fields Full Load Error (Prod)!"
...
try:
    ...
    TEXT = "Hi BI Team, Zendesk Organization Fields Full Load Failed - Database Connection Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    ...
    job_name = 'Organization_Fields_Full_Load'
...
# Insert new row to job details log table
    ...
    conn.commit()
try:
    url = 'https://dynatrace.zendesk.com/api/v2/organization_fields.json?page[size]=100'
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Organization Fields Full Load Failed - Zendesk API Connection Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
...
try:
    while url:
        ...
        data = response.json ()
        organization_fields = data['organization_fields']
        df = pd.DataFrame ( organization_fields )
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Organization Fields Full Load Failed - Fetch Data From API Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
    ...
    cur.execute("insert into logging.job_details_logs(job_execution_id,step_name,exec_timestamp, number_of_records, status, message) values ("+job_exec_id+", 'fetch data from api', current_timestamp,"+ str(len(df)) +", 'Success', NULL)")
    conn.commit()

try:
    cur.execute ( "truncate table int_zendesk.zendesk_organizations_fields" )
    df_data.to_sql('zendesk_organizations_fields', cnx, schema = 'int_zendesk', method='multi',if_exists='append',index=False)
except:
    cur.execute ("update logging.job_summary_logs set status ='Failed', end_time = current_timestamp where job_execution_id = " + job_exec_id )
    ...
    conn.commit ()
    TEXT = "Hi BI Team, Zendesk Organization Fields Full Load Failed - Data Loading Process need to be fixed, try to check dataframe data type or check if the object in the database has been used in any view."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
