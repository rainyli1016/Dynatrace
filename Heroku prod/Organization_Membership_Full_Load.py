...
SUBJECT = "Zendesk Organization Membership Full Load Error (Prod)!"
...
try:
    ...
    TEXT = "Hi BI Team, Zendesk Organization Membership Full Load Failed - Database Connection Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    ...
    job_name = 'Organization_Membership_Full_Load'
...
# Insert new row to job details log table
    ...
    conn.commit()
try:
    url = 'https://dynatrace.zendesk.com/api/v2/organization_memberships.json?page[size]=100'
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Organization Membership Full Load Failed - Zendesk API Connection Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
...
try:
    while url:
        ...
        df = pd.DataFrame(organization_memberships)
    ...
    conn.commit ()
    TEXT = "Hi BI Team, Zendesk Organization Membership Full Load Failed - Fetch Data From API Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format(SUBJECT, TEXT)
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
    ...
    conn.commit ()
try:
    cur.execute ("truncate table int_zendesk.zendesk_organizations_memberships")
    df_data.to_sql ( 'zendesk_organizations_memberships', cnx, schema='int_zendesk', method='multi', if_exists='append', index=False )
except:
    cur.execute("update logging.job_summary_logs set status ='Failed', end_time = current_timestamp where job_execution_id = " + job_exec_id )
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Organization Membership Full Load Failed - Loading Data to Database Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
