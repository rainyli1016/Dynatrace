...
SUBJECT = "Zendesk Group Full Load Error (Prod)!"
...
try:
    ...
    TEXT = "Hi BI Team, Zendesk Group Full Load - Database Connection Issue need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
...
# Insert new row to job details log table
    ...
    conn.commit()
try:
    url = 'https://dynatrace.zendesk.com/api/v2/groups.json?page[size]=100'
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Group Full Load - Zendesk API Connection need to be Fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
...
try:
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Group Full Load - Fetch data from API failed and need to be fixed."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
...
try:
    cur.execute ("truncate table int_zendesk.zendesk_groups")
    df_data.to_sql('zendesk_groups', cnx, schema = 'int_zendesk', method='multi',if_exists='append',index=False)
    ...
    conn.commit()
    TEXT = "Hi BI Team, Zendesk Group Full Load - Loading Zendesk API data to database failed and needs to be fixed, try to compare data type from API with database."
    message = 'Subject: {}\n\n{}'.format ( SUBJECT, TEXT )
    s.login ( gmail_user, gmail_password )
    s.sendmail ( "gmail_user", "rainy.li@dynatrace.com", message )
    s.sendmail ( "gmail_user", "Bussys_bi@dynatrace.com", message )
